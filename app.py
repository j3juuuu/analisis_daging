...
def compute_lbp_feature(gray):
    ...
    uniform_prop = float(hist[0]) if len(hist) > 0 else 0.0
...
def compute_shape_features(gray):
    ...
    convexity = float(area) / float(hull_area) if hull_area > 0 else 0
...
def detect_blobs(gray):
    ...
    mean_blob_area = float(np.mean(areas)) if len(areas) > 0 else 0.0
...
# CLASSIFICATION
# =========================
def classify_freshness(h, s, contrast, homogeneity, blob_count):
    score = 0
    explanation = []

    if 0 <= h <= 40:
        score += 1
        explanation.append("Hue mendekati warna daging segar.")

    if s > 40:
        score += 1
        explanation.append("Saturasi warna cukup baik.")

    if homogeneity > 0.5:
        score += 1
        explanation.append("Tekstur relatif homogen.")

    if contrast < 60:
        score += 1
        explanation.append("Permukaan tidak terlalu kasar.")

    if blob_count < 5:
        score += 1
        explanation.append("Jumlah bercak relatif sedikit.")
    ...
    if request.method == "POST":
        ...
        if not file:
            ...
            if img is None:
                ...
                mean_lbp, uniform_lbp = compute_lbp_feature(gray)
                ...
                status, explanation = classify_freshness(
                    h, s, contrast, homogeneity, blob_count
                )
                ...
                result = {
                    ...
                    "mean_blob_area": round(mean_blob_area, 2),
